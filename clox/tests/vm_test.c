...
void testItShouldBeAbleToGetValueInImmediatelyEnclosingFunction()
{
    const char *sourceCode = "{ func foo() { var a = 1; func bar() {print a; } bar(); } foo();}";
    runInterpreter(&testObject, sourceCode);
    TEST_ASSERT_EQUAL(1, test_messages_size);
    TEST_ASSERT_EQUAL_STRING("1.000000", test_messages[0]);
}
...
int main(void)
{
    ...
    RUN_TEST(testItShouldBeAbleToGetValueInImmediatelyEnclosingFunction);
    return UNITY_END();
}